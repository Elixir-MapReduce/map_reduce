defmodule Master do
  ...
  def init([worker_cnt, splits_count, caller]) do   
    {:ok,
     %{
       worker_pool: Enum.map(1..worker_cnt, fn _index -> GenServer.start_link(Worker, [self()]) |> elem(1) end),
  ...
  @impl true
  def handle_info({:job_done, _response, worker_pid}, %{remaining_jobs: remaining_jobs, queue: queue, worker_pool: pool, caller: caller} = state) do
  ...
  def handle_cast({:reduce_phase, name_in_cache, _id}, %{worker_pool: pool, queue: queue} = state) do
    {queue, pool} = execute_or_put_in_queue(:reduce, name_in_cache, Enum.empty?(pool), pool, queue)
    {:noreply, %{state | worker_pool: pool, queue: queue}}
  end
